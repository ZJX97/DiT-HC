...
mpiProfiler=False
torchProfiler=False
dataset="mid" #["mini"|"mid"|"all"]

sudo cpupower frequency-set -f 1.55GHz
...
if [ ${mpiProfiler} = True ]; then
	...
	export MPI_PROF_LOGPATH=./mpi_log/4-10/1n4p/
fi
...
taskset -c $be-$en python -u "${ROOT_DIR}/mpi_train/train.py" \
    ...
    --profiler-dir ${ROOT_DIR}/mpi_train/profiler_log/${mode}/0407 \
    --global-seed 42 \
    --pre-latent \
    ${opt}
    #--debug
