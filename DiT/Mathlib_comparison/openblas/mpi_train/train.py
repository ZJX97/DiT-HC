...
#    torch.backends.mkldnn.enabled=True   
    ...
    torch.ops.load_library("/pacific_ext/liujt/test_workspace/DiT_zjx/libkpprim-0331-linear.so")
    ...
    if args.debug:
        ...
        if rank == 0:
            for sorted_key in ["cpu_time_total", "self_cpu_time_total", "cpu_memory_usage", "self_cpu_memory_usage"]:
                with open('/pacific_ext/liujt/test_workspace/DiT_zjx_linear_baseline/mpi_train/output_withProf/torch_profiler', 'a', encoding='utf-8') as f:
                    print(f"==========================sorted by {sorted_key}==========================", file=f)
                    print(prof.key_averages().table(sort_by=sorted_key, row_limit=40), file=f)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--profiler-dir", type=str, default="./profiler_res/0401")
