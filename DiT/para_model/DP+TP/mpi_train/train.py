...
#    torch.backends.mkldnn.enabled=True   
    ...
    if tp_size > 1:
        ...
        for block in model.blocks:
            ...
            gd = math.gcd(block.attn.num_heads, tp_size)
            block.attn.num_heads = block.attn.num_heads // gd
            block.attn.head_dim = block.attn.head_dim // ((tp_size) // gd)
    ...
    if args.debug:
        ...
        if rank == 0:
            for sorted_key in ["cpu_time_total", "self_cpu_time_total", "cpu_memory_usage", "self_cpu_memory_usage"]:
                with open(f"{args.profiler_dir}/log", 'a', encoding='utf-8') as f:
