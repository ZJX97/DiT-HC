...
constexpr const char* MPI_BACKEND_NAME = "hmpi";
...
class TORCH_API ProcessGroupHMPI : public Backend {
 public:
  class WorkHMPI : public Work {
   public:
    explicit WorkHMPI(
   ...
   protected:
    friend class ProcessGroupHMPI;

   private:
    void finishWorkHMPI();
    void finishWorkHMPIError(const std::exception_ptr& eptr);
  ...
  //explicit ProcessGroupHMPI(int rank, int size, MPI_Comm pgComm);
  explicit ProcessGroupHMPI(int rank, int size, void* pgComm);

  ~ProcessGroupHMPI() override;
  ...
  // Creating a new ProcessGroupHMPI, will initialize MPI if not initialized
  //static c10::intrusive_ptr<ProcessGroupHMPI> createProcessGroupHMPI(
  ...
  static c10::intrusive_ptr<ProcessGroupHMPI> createProcessGroupHMPI(
      ...
      //const c10d::DistributedBackendOptions& opts, const c10::intrusive_ptr<Options>& pg_options );
  ...
  //ADD*********************************
  //static void ProcessGroupHMPIConstructor() __attribute__((constructor)) {
  //  py::object module = py::module::import("torch.distributed");
  //  py::object register_backend = 
  //      module.attr("Backend").attr("register_backend");
  //  register_backend("hmpi", py::cpp_function(createProcessGroupHMPI), true, "cpu");
  //}
  static void ProcessGroupHMPIConstructor() __attribute__((constructor)) {
    py::object module = py::module::import("torch.distributed");
    py::object register_backend = 
    ...
    //py::object register_backend = 
    //    module.attr("Backend").attr("register_backend");
    //register_backend("hmpi", 
    //    py::cpp_function([](const c10d::DistributedBackendOptions& opts){
    //        std::vector<int> ranks;
    //        return ProcessGroupHMPI::createProcessGroupHMPI(ranks);
    //    }),
    //    true, "cpu");
 ...
 protected:
  using WorkType =
      std::tuple<std::unique_ptr<WorkEntry>, c10::intrusive_ptr<WorkHMPI>>;
  // Worker thread loop
  void runLoop();
...
// ProcessGroupHMPI implements MPI bindings for c10d.
//
...
// Worker thread. Therefore, ProcessGroupHMPI requires the MPI implementation
...
// MPI_THREAD_SERIALIZED, ProcessGroupHMPI will only support a singe process
// group. In other words, no more than 1 process group can be created globally.
//
// If you would like to use multiple ProcessGroupHMPI, it requires your MPI
...
//
// Also note that ProcessGroupHMPI only supports a single Tensor operation. In
...
// ProcessGroupHMPI will automatically detect this support.
